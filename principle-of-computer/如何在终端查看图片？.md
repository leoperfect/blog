...
前提是linux系统或者是mac系统，windows系统可以不要往下看了。

先上效果：

![效果图](https://github.com/leoperfect/blog/raw/master/pic/imgcat-result.png)

再上步骤：

1. 在`home`目录下新建一个`bin`目录（个人一般喜欢把一些脚本统一放在这里），并创建一个名为`imgcat`的文件，并将下列内容复制到文件中并保存。
...
```bash
$ sudo chmod 777 ./imgcat
```

3. 将`~/bin`添加到环境变量中

```bash
// 用的是bash就修改.bashrc 用的是zsh就修改.zshrc
$ vi ~/.bashrc
// 添加如下内容
export PATH = $PATH:/home/yourname/bin
```

4. 重启bash使环境变量生效，或者通过`source`来使其生效
